...
int skip_headers(struct file_info *fi)
{
  ...
  while ((iline = getline_new(fi)) != NULL)
    if (iline[0] != '#')
      break;
...
struct data_entry *old_randomize_entry_order(struct data_entry *entry)
{
  long i, nol;
...
int set_vector_functions(struct teach_params *params, char *name)
{
...
int set_teach_params(struct teach_params *params, struct entries *codes, 
		     struct entries *data, long dbuffer, char *name)
{
  int error = 0;
