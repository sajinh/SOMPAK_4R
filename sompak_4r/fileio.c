...
/* def. of getline conflicts with that in stdio.h*/
/* I renamed *getline to getline_new ; saji apr16 */
char *getline_new(struct file_info *fi) 
{
  ...
  while (1)
      ...
      if (len >= strl)
	{
	  ...
	  if (strl > ABS_STR_LNG) 
	    {
	      fprintf(stderr, "getline_new: Too long lines in file %s (max %d)\n",
		      fi->name, ABS_STR_LNG);
      ...
      if (c == EOF)
	{
	  ...
	  if (feof(fp))
	  ...
	  else
	    {
	      ...
	      fprintf(stderr, "getline_new: read error on line %ld of file %s\n", fi->lineno, fi->name);
	      perror("getline_new");
	    }
	  break;
	}
...
/* rewind_file - go to the beginning of file. If file is an ordinary
   file, seeks to the start of file.  If the file is a compressed
   file, closes the old file and runs the uncompressing command
